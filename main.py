...
async def cmd_start(message: Message, state: FSMContext):
    ...
    try:
        ...
        if user and str(user.status).strip().lower() in ["очікує", "pending"]:
            await message.answer("⏳ Ваша заявка скоро активується. Очікуйте підтвердження та повідомленян від нас!")
            return
...
# ==========================================
async def main():
    print("Бот із модулями FSM-онбордингу та медіа запущений...")
